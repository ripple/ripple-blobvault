    ...
    Message.verifyMessageSignature_RPC(
        ...
        function(err,resp) {
            ...
            if (err) {
                ...
                if (err == 'no response') {
                    handleError({res:res, error:new Error('No response from rippled on ECDSA'), statusCode:401});
                } else 
                    handleError({res:res, error:new Error('Unable to validate: Ripple Network error'), statusCode:401});
            } else if (resp === false) {
