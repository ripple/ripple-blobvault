...
[
  ...
  json.push({
    "type": "small",
    "text": "\nYou are receiving this email because you created a RippleID account.\t"
          + "If you did not sign up for a RippleID account, please disregard this email.",
  });
  json.push({type: "hr"});
  json.push({
    "type": "small",
...
function emailToHTML(tmpl) {
    ...
    return '<table cellspacing="0" cellpadding="0" border="0" width="440" style="font-family:sans-serif;color:#222222;padding-left:40px;">'
         ...
         +     '<br>'
         +     '<img src="' + idHost + '/assets/images/rippleid-logo.png" alt="RippleID">'
         +     '<br><br>'
         +     '<div>'
         +       json.join('').replace(/\n/g, "<br>").replace(/\t/g, " ")
         +     '</div>'
