...
var create = function(params,cb) {
    ...
    .catch(function(e) {
        console.log("Got Error create " , e)
    });
}
...
var read = function(params, cb) {
    ...
    if (typeof username !== 'string') {
        process.nextTick(function() {
            throw { error : new Error("No username supplied to postgres read"), res : res}
        });
        return;
    }
    db('blob')
    ...
    .catch(function (obj) {
        handleError(obj);
    // TODO
    // bluebird domains issue.. need to thread in domain, set common domain in server, thread through
//        d.emit('error', {error:e.error, res: res});
...
exports.blobPatch = function(req,res,cb) {
    ...
    // XXX Check quota
    console.log("BLOB PATCH", req.body);
    db('blob')
    ...
    .catch(function(obj) {
        console.log("WOW SOME ERROR:", obj);
...
exports.blobDelete = function(req,res,cb) {
    ...
    .then(function() {
        console.log('Success Blob Deleted !!');
        cb({result:success});
    })
    .catch(function(obj) {
        handleError(obj);
...
exports.hmac_getSecret = function(blobId, callback) {
    db('blob')
    .where('id','=',blobId)
    .select('auth_secret')
    .then(function(rows) {
